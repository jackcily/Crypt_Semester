...
// function registerInDb($name, $email, $password, $pubkey, $privkey) {
function registerInDb($name, $password, $pubkey, $privkey,$email) {
    try {
        $conn = connectDb();
        $sql = "INSERT INTO users (name, password, pubkey, privkey,email) VALUES (:name,  :password, :pubkey, :privkey, :email)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':name', $name);
